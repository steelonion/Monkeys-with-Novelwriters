...
{{
  ...
  "characters": {{
    "角色名": {{
      ...
      "inventory": ["物品1"],
      "notes": "备注",
      "custom_fields": {{"技能/等级/属性等自定义字段": "值"}}
    }}
  }}
}}
```
...
7. 角色的 custom_fields 用于存放不属于基础字段的自定义属性（如技能、等级、属性值、阵营、能力系统、修为境界、血统、法术列表、好感度等）。**请积极从文本中识别并创建合适的 custom_fields**——只要某项信息无法被 name/description/appearance/outfit/personality/status/location/relationships/inventory/notes 充分表示，就应该为它建立一个语义清晰的 custom_field 键。例如：修仙小说可创建"修为境界"、"功法"、"灵根"；游戏类可创建"等级"、"HP"、"MP"、"技能列表"等。
...
def _build_characters_block(characters: dict[str, CharacterState]) -> str:
    ...
    for name, char in characters.items():
        ...
        if char.custom_fields:
            for cf_key, cf_val in char.custom_fields.items():
                if isinstance(cf_val, (list, dict)):
                ...
                if isinstance(char_info, dict):
                    try:
                        characters[char_name] = CharacterState(
                            ...
                            custom_fields=char_info.get("custom_fields", {}),
                        )
                    except Exception:
